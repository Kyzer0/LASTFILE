...
function validateStudentCount() {
    ...
    if (!studentCountValue.match(/^\d+$/) || parseInt(studentCountValue) <= 0) {
        studentCountError.innerHTML = '<p>Student count must be a positive number</p>'
        return false
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateSectionNameDashboard() {
       ...
       // Same for dashboard version
       function validateStudentCountDashboard() {
        const studentCountValue = studentCountDashboard.value.trim();
    
        if (!studentCountValue.match(/^\d+$/) || parseInt(studentCountValue) <= 0) {
            studentCountErrorDashboard.innerHTML = '<p>Student count must be a positive number</p>';
            return false;
        }
